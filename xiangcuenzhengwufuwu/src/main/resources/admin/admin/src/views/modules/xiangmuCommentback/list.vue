        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('xiangmuCommentback','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('xiangmuCommentback','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('xiangmuCommentback','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xiangmuCommentback','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiangcuenzhengwufuwu/upload/xiangmuCommentbackMuBan.xls"
                        >批量导入预约评价数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xiangmuCommentback','导入导出')"
                                ...
                                :on-success="xiangmuCommentbackUploadSuccess"
                                :on-error="xiangmuCommentbackUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xiangmuCommentback','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入预约评价数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xiangmuCommentback','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xiangmuCommentback.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('xiangmuCommentback','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xiangmuCommentbackText"
                                      header-align="center"
                                      label="评价内容">
                        <template slot-scope="scope">
                            <span v-if="scope.row.xiangmuCommentbackText != null &&scope.row.xiangmuCommentbackText.length>10">
                                {{scope.row.xiangmuCommentbackText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.xiangmuCommentbackText}}
                            </span>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('xiangmuCommentback','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xiangmuCommentback','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>
                            <el-button v-if="isAuth('xiangmuCommentback','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            getDataList() {
                ...
                params['xiangmuCommentbackDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "xiangmuCommentback/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            xiangmuCommentbackUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xiangmuCommentback/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            xiangmuCommentbackUploadError(data){
