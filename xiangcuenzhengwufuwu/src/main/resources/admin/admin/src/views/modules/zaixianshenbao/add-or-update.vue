    ...
    <div class="addEdit-block">
        <el-form
            ...
            <el-row>
                ...
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="申报项目" prop="zaixianshenbaoTypes">
                        <el-select v-model="ruleForm.zaixianshenbaoTypes" :disabled="ro.zaixianshenbaoTypes" placeholder="请选择申报项目">
                            <el-option
                                v-for="(item,index) in zaixianshenbaoTypesOptions"
                    ...
                    <div v-else>
                        <el-form-item class="input" label="申报项目" prop="zaixianshenbaoValue">
                        <el-input v-model="ruleForm.zaixianshenbaoValue"
                            placeholder="申报项目" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
    ...
    export default {
        data() {
            return {
                ...
                yonghuForm: {},
                ro:{
                    zaixianshenbaoTypes: false,
                    yonghuId: false,
                    zaixianshenbaoYesnoTypes: false,
                },
                ruleForm: {
                    zaixianshenbaoTypes: '',
                    yonghuId: '',
                    zaixianshenbaoYesnoTypes: '',
                },
                zaixianshenbaoTypesOptions : [],
                zaixianshenbaoYesnoTypesOptions : [],
                yonghuOptions : [],
                rules: {
                   zaixianshenbaoTypes: [
                              { required: true, message: '申报项目不能为空', trigger: 'blur' },
                              {  pattern: /^[1-9][0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
                   ...
                   yonghuId: [
                              { required: true, message: '申报用户不能为空', trigger: 'blur' },
                              {  pattern: /^[1-9][0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=zaixianshenbao_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.zaixianshenbaoTypesOptions = data.data.list;
                    }
                });
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=zaixianshenbao_yesno_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.zaixianshenbaoYesnoTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`zaixianshenbao/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.zaixianshenbaoCrossAddOrUpdateFlag = false;
                                        this.parent.search();
